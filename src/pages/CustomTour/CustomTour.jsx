...
const [totalHotelPrice, setTotalHotelPrice] = useState(0);
  ...
  const totalPrice = selectedHotel ? selectedHotel.price * numRooms * numPersons : 0;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="bg-white p-8 rounded-3xl shadow-lg backdrop-blur-md bg-opacity-80 mt-6">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="p-4 rounded-lg bg-gray-50 sha">
          ...
          <div className="p-4 rounded-lg bg-gray-50">
            <label className="font-semibold text-gray-700 mb-2 block">To</label>
            <FormControl fullWidth>
              <Select
                value={toLocation}
      ...
      {/* Hotel Section */}
      <div className="p-4 rounded-lg bg-gray-50">
  <label className="font-semibold text-gray-700 mb-2 block">Select Hotel</label>
  <FormControl fullWidth>
    <Select
        ...
        hotelList.map((hotel, index) => (
          <MenuItem key={index} value={hotel.name}>
            <div className="flex items-center gap-3">
              <img src={hotel.image} alt={hotel.name} className="w-10 h-10 rounded-md" />
              <span>{hotel.name} - ${hotel.price}/night</span>
            </div>
          </MenuItem>
        ))
      ) : (
...
{selectedHotel && (
  <div className="card bg-base-100 shadow-xl mt-6 p-4 flex flex-col sm:flex-row justify-between">
    ...
    <div className="flex items-center gap-6">
      <img src={selectedHotel.image} className="w-32 h-32 object-cover rounded-lg" alt={selectedHotel.name} />
      <div>
        <h2 className="text-2xl font-bold">{selectedHotel.name}</h2>
        <p className="text-gray-600">
          Price: <span className="font-semibold">${selectedHotel.price}/night</span>
        </p>
      </div>
    </div>
    ...
    <div className="flex flex-col gap-3">
      ...
      {/* Total Price Display */}
      <div className="mt-3">
        <h3 className="text-lg font-bold">Total Price: ${numRooms*numPersons*selectedHotel.price}</h3>
      </div>
    </div>
  </div>
)}
              ...
              <FormControl fullWidth>
                <div className="space-y-2">
                  <div className="text-center">
                    <p className="text-xl font-bold">${totalTransportPrice}</p>
      ...
      <div className="card bg-base-100 shadow-xl">
        <div className="card-body">
          <h2 className="card-title mb-4 text-2xl font-bold">Rental Section</h2>
          <div className="flex items-center justify-between mb-4">
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div className="text-lg bg-white p-6 rounded-2xl shadow-md border border-gray-100">
      <p className="flex justify-between items-center border-b pb-2 text-gray-700">
        Hotel Cost Total: <span className="font-bold text-xl text-green-700">${totalPrice}</span>
      </p>
      <p className="flex justify-between items-center border-b pb-2 text-gray-700">
        Transportation Total: <span className="font-bold text-xl text-blue-700">${effectiveTransportTotal}</span>
      </p>
      <p className="flex justify-between items-center border-b pb-2 text-gray-700">
        Rental Total: <span className="font-bold text-xl text-purple-700">${needRental ? rentalNumeric : 0}</span>
      </p>
      <p className="flex justify-between items-center mt-4 text-xl font-semibold text-gray-900">
        Final Total: <span className="text-green-900 text-2xl">${totalPrice + effectiveTransportTotal + (needRental ? rentalNumeric : 0)}</span>
      </p>
    </div>
